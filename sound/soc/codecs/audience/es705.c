...
#elif defined(CONFIG_MACH_KACTIVELTE_EUR)
...
int es705_put_veq_block(int volume)
{
	...
	cmd = cpu_to_le32(cmd);
	ret = es705->dev_write(es705, (char *)&cmd, 4);
	dev_dbg(es705->dev, "%s(): writing cmd 0x%08x to device - VEQ Main gain\n",
		    __func__, cmd);
	if (ret < 0) {
		dev_err(es705->dev, "%s(): error writing cmd 0x%08x to device\n",
		    __func__, cmd);
	...
	cmd = cpu_to_le32(cmd);
	ret = es705->dev_write(es705, (char *)&cmd, 4);
	dev_dbg(es705->dev, "%s(): writing cmd 0x%08x to device (%d)\n",
		    __func__, cmd, volume);	
	if (ret < 0) {
		dev_err(es705->dev, "%s(): error writing cmd 0x%08x to device\n",
		    __func__, cmd);
	...
	cmd = cpu_to_le32(cmd);
	ret = es705->dev_write(es705, (char *)&cmd, 4);
	dev_dbg(es705->dev, "%s(): writing cmd 0x%08x to device - VEQ Adjust\n",
		    __func__, cmd);	
	if (ret < 0) {
		dev_err(es705->dev, "%s(): error writing cmd 0x%08x to device\n",
		    __func__, cmd);
	...
	cmd = cpu_to_le32(cmd);
	ret = es705->dev_write(es705, (char *)&cmd, 4);
	dev_dbg(es705->dev, "%s(): writing cmd 0x%08x to device - Filter\n",
		    __func__, cmd);
	if (ret < 0) {
		dev_err(es705->dev, "%s(): error writing cmd 0x%08x to device\n",
		    __func__, cmd);
	...
	if (network_type == WIDE_BAND)
		...
		ret = es705->dev_write(es705, (char *)&veq_coefficients_nb[volume][0],
					veq_coeff_size);
	if (ret < 0) {
		dev_err(es705->dev, "%s(): wdb error\n", __func__);
		goto EXIT;
	}

	dev_info(es705->dev, "%s(): wdb success\n", __func__);
EXIT:
	mutex_unlock(&es705->api_mutex);
	if (ret != 0)
		dev_err(es705->dev, "%s(): wdb failed ret=%d\n",
			__func__, ret);
	return ret;
}
