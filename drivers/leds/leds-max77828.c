...
static int max77828_led_get_en_value(struct max77828_led_data *led_data, int on)
{
        int mode;
        int ret;

        if (on)
                mode = 0x03; /*triggered via MAX77828_TORCHEN_OR_FLASHSTB mode*/
        else {
                if (led_data->data->id == MAX77828_FLASH)
                        mode = 0x02; /*Flash triggered via FLASHEN*/
                else
                        mode = 0x01; /*Torch triggered via TORCHEN*/
        ...
        DEBUG_MAX77828("[LED] %s: led mode: 0x%x, on: %d\n", __func__, mode, on);
...
static void max77828_flash_set(struct max77828_led_data *led_data)
{
	int ret;
	int value;
	enum led_brightness brightness = led_data->test_brightness;
...
//		ret = max77828_write_reg(led_data->i2c, MAX77828_LED_REG_MODE_SEL, 0x42);
		if(IS_ERR_VALUE(ret))
			pr_err("(%s) :  MAX77828_LED_REG_MODE_SEL update failed\n",__func__);
...
static void max77828_torch_set(struct max77828_led_data *led_data)
{
	...
	if(brightness == LED_OFF)
	{
		...
		/* Torch OFF */
		ret = max77828_update_reg(led_data->i2c, MAX77828_LED_REG_I_TORCH1,
				0, MAX77828_TORCH_FLED1_EN);
		if(IS_ERR_VALUE(ret))
...
static void max77828_rgb_set(struct max77828_led_data *led_data)
{
	...
	else
	{
		...
		/* LEDBLNK ON */
		ret = max77828_write_reg(led_data->i2c, MAX77828_LED_REG_LEDBLNK, 0xFF);
		if(IS_ERR_VALUE(ret))
			pr_err("(%s) :  LED Bit %x update failed\n",__func__, bit );

		/* LEDRMP ON */
		ret = max77828_write_reg(led_data->i2c, MAX77828_LED_REG_LEDRMP, 0xFF);
		if(IS_ERR_VALUE(ret))
			pr_err("(%s) :  LED Bit %x update failed\n",__func__, bit );
