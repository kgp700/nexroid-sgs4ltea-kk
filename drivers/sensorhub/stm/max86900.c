...
#define MAX86900A_DEFAULT_CURRENT	0x59
...
static int max86900_init_device(struct max86900_device_data *data)
{
	...
	if (data->part_type == PART_TYPE_MAX86900A) {
		...
		/*
		err = max86900_write_reg(data, MAX86900_TEST_ENABLE_PLETH, 0x04);
		if (err != 0) {
			pr_err("%s - error initializing MAX86900_TEST_ENABLE_PLETH!\n",
				__func__);
			return -EIO;
		}
		*/
...
static int max86900_read_data(struct max86900_device_data *device, u16 *data)
{
	...
	if (device->sample_cnt == 3 && device->led == 0) {
...
/* test sysfs */
static int max86900_set_idac(struct max86900_device_data *data)
{
	int err;
	...
	/* Set IDAC Gain */
	err = max86900_write_reg(data, MAX86900_TEST_IDAC_GAIN, data->idac);
	if (err != 0) {
		pr_err("%s - error initializing MAX86900_TEST_IDAC_GAIN!\n",
			__func__);
		return -EIO;
	}
...
{
	int err;
	/* 46.9bpm:0x47, 78.3bpm:0x4A, 93.8bpm:0x4E, 187.5bpm:0x51 Support */
	/* MAX86900_SPO2_CONFIGURATION Setting */
	err = max86900_write_reg(data, MAX86900_SPO2_CONFIGURATION,
		data->hr_range);
	if (err != 0) {
		pr_err("%s - error initializing MAX86900_SPO2_CONFIGURATION!\n",
			__func__);
...
static int max86900_set_hr_range2(struct max86900_device_data *data)
{
	...
	/* Set PLETH */
	err = max86900_write_reg(data, MAX86900_TEST_ENABLE_PLETH,
		data->hr_range2);
	if (err != 0) {
		pr_err("%s - error initializing MAX86900_TEST_ENABLE_PLETH!\n",
			__func__);
...
static int max86900_set_look_mode(struct max86900_device_data *data)
{
	int err;
	/* Entering Test Mode1 */
	err = max86900_write_reg(data, MAX86900_TEST_MODE, 0x54);
	...
	/* Set LOOK Mode */
	if (data->look_mode)
		err = max86900_write_reg(data, MAX86900_TEST_ROUTE_MODULATOR, 0x4B);
	else
		err = max86900_write_reg(data, MAX86900_TEST_ROUTE_MODULATOR, 0x0B);

	if (err != 0) {
		pr_err("%s - error initializing MAX86900_TEST_ROUTE_MODULATOR!\n",
			__func__);
		return -EIO;
	}
	...
	pr_info("%s - look mode = %u\n", __func__, data->look_mode);
	return 0;
}
...
static int max86900_set_look_mode_ir(struct max86900_device_data *data)
{
	...
	/* Set LOOK Mode IR */
	err = max86900_write_reg(data, MAX86900_TEST_LOOK_MODE_IR, data->look_mode_ir);
	if (err != 0) {
		pr_err("%s - error initializing MAX86900_TEST_LOOK_MODE_IR!\n",
			__func__);
		return -EIO;
	}
	/* Exiting Test Mode */
...
static int max86900_set_look_mode_red(struct max86900_device_data *data)
{
	...
	/* Set LOOK Mode RED */
	err = max86900_write_reg(data, MAX86900_TEST_LOOK_MODE_RED, data->look_mode_red);
	if (err != 0) {
		pr_err("%s - error initializing MAX86900_TEST_LOOK_MODE_RED!\n",
			__func__);
		return -EIO;
	}
	/* Exiting Test Mode */
...
static int max86900_eol_test_enable(struct max86900_device_data *data)
{
	...
	/* Enable IDAC */
	err = max86900_write_reg(data, MAX86900_TEST_ENABLE_IDAC, 0x10);
	if (err != 0) {
		pr_err("%s - error initializing MAX86900_TEST_ENABLE_IDAC!\n",
			__func__);
		return -EIO;
	}

	/* Enable PLETH test mode - default 46bpm */
	err = max86900_write_reg(data, MAX86900_TEST_ENABLE_PLETH, 0x38);
	...
	/* Routing IDAC to modulate IR/RED */
	err = max86900_write_reg(data, MAX86900_TEST_ROUTE_MODULATOR, 0x0B);
	if (err != 0) {
		pr_err("%s - error initializing MAX86900_TEST_ROUTE_MODULATOR!\n",
			__func__);
	...
	/* Set IDAC Gain */
	err = max86900_write_reg(data, MAX86900_TEST_IDAC_GAIN, 0x18);
	if (err != 0) {
		pr_err("%s - error initializing MAX86900_TEST_IDAC_GAIN!\n",
			__func__);
		return -EIO;
...
static void max86900_eol_test_onoff(struct max86900_device_data *data, int onoff)
{
	...
	if (onoff) {
		...
		data->hr_range = 0x47;
		data->led_current = data->default_current; /* set default led_current 68mA */
...
static ssize_t idac_range_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int err;
	struct max86900_device_data *data = dev_get_drvdata(dev);

	err = kstrtou8(buf, 10, &data->idac);
	if (err < 0)
		return err;

	err = max86900_set_idac(data);
	if (err < 0)
		return err;
...
static ssize_t idac_range_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct max86900_device_data *data = dev_get_drvdata(dev);
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("%s - hr_range2 = %x\n", __func__, data->hr_range2);

	return snprintf(buf, PAGE_SIZE, "%u\n", data->hr_range2);
}

static ssize_t look_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int err;
	struct max86900_device_data *data = dev_get_drvdata(dev);

	err = kstrtou8(buf, 10, &data->look_mode);
	if (err < 0)
		return err;

	err = max86900_set_look_mode(data);
	if (err < 0)
		return err;
...
static ssize_t look_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct max86900_device_data *data = dev_get_drvdata(dev);

	pr_info("%s - look_mode = %x\n", __func__, data->look_mode);
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("%s - eol_test_is_enable = %u\n",
...
static DEVICE_ATTR(idac_range, S_IRUGO | S_IWUSR | S_IWGRP,
	idac_range_show, idac_range_store);
...
static DEVICE_ATTR(look_mode, S_IRUGO | S_IWUSR | S_IWGRP,
	look_mode_show, look_mode_store);
...
int max86900_probe(struct i2c_client *client, const struct i2c_device_id *id )
{
	...
	data->hr_range = 0x47; /* set default hr_range 46.9bpm */
	data->led_current = data->default_current; /* set default led_current 68mA */
	...
	data->client->addr = MAX86900A_SLAVE_ADDR; /* Max86900A check first */

	buffer = MAX86900_WHOAMI_REG;
	err = max86900_read_reg(data, &buffer, 1);
	if (err) {
		/* MAX86900 */
		data->client->addr = MAX86900_SLAVE_ADDR; /* Max86900 check later */
		buffer = MAX86900_WHOAMI_REG;
		err = max86900_read_reg(data, &buffer, 1);
		...
		pr_info("%s - MAX86900 (0x%X)\n", __func__, MAX86900_SLAVE_ADDR);
	} else {
		/* MAX86900A */
		if (buffer != MAX86900A_WHOAMI) {
			/*
			pr_err("%s WHOAMI value (0x%x)\n", __func__, buffer);
			err = -ENODEV;
			goto err_of_node;
		...
		pr_info("%s - MAX86900A (0x%X)\n", __func__, MAX86900A_SLAVE_ADDR);
	}
