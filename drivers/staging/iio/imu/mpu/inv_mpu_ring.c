...
int set_inv_enable(struct iio_dev *indio_dev, bool enable)
{
	...
	u8 d;
	struct motion_int_data *mot_data = &st->mot_data;
#endif
	...
	if (st->reactive_enable) {
		if (!mot_data->is_set) {
			inv_i2c_read(st, REG_PWR_MGMT_1, 1, &st->mot_data.pwr_mgmt[0]);
			inv_i2c_read(st, REG_PWR_MGMT_2, 1, &st->mot_data.pwr_mgmt[1]);
			inv_i2c_read(st, REG_CONFIG, 1, &st->mot_data.config);
			inv_i2c_read(st, REG_ACCEL_CONFIG, 1, &st->mot_data.accel_config);
			inv_i2c_read(st, REG_6500_ACCEL_CONFIG2, 1, &st->mot_data.accel_config2);
			inv_i2c_read(st, REG_INT_ENABLE, 1, &st->mot_data.int_enable);
			inv_i2c_read(st, REG_6500_LP_ACCEL_ODR, 1, &st->mot_data.lp_accel_odr);
			inv_i2c_read(st, REG_6500_ACCEL_INTEL_CTRL, 1, &st->mot_data.accel_int_ctrl);
			inv_i2c_read(st, REG_6500_ACCEL_WOM_THR, 1, &st->mot_data.motion_threshold);
			inv_i2c_read(st, REG_GYRO_CONFIG, 1, &st->mot_data.gyro_config);

			mot_data->is_set = true;
		}
		...
		/* put gyro in standby. */
		if (!(st->chip_config.gyro_enable))
			inv_i2c_single_write(st, REG_PWR_MGMT_2, (0x04|0x02|0x01));
		...
		if (mot_data->is_set) {
			inv_i2c_single_write(st, REG_PWR_MGMT_1, mot_data->pwr_mgmt[0]);
			msleep(50);
			inv_i2c_single_write(st, REG_PWR_MGMT_2, mot_data->pwr_mgmt[1]);
			inv_i2c_single_write(st, REG_CONFIG, mot_data->config);
			inv_i2c_single_write(st, REG_ACCEL_CONFIG,
					mot_data->accel_config);
			inv_i2c_single_write(st, REG_6500_ACCEL_CONFIG2,
					mot_data->accel_config2);
			inv_i2c_single_write(st, REG_GYRO_CONFIG,
					mot_data->gyro_config);
			inv_i2c_single_write(st, REG_INT_ENABLE,
					mot_data->int_enable);
			inv_i2c_single_write(st, REG_6500_LP_ACCEL_ODR,
					mot_data->lp_accel_odr);
			inv_i2c_single_write(st, REG_6500_ACCEL_INTEL_CTRL,
					mot_data->accel_int_ctrl);
			inv_i2c_single_write(st, REG_6500_ACCEL_WOM_THR,
					mot_data->motion_threshold);
			inv_i2c_single_write(st, REG_SAMPLE_RATE_DIV,
					mot_data->smplrt_div);

			pr_info("[SENSOR] %s: disable reactive config\n", __func__);
			mot_data->is_set = false;
		}
	}
#endif
	return 0;
