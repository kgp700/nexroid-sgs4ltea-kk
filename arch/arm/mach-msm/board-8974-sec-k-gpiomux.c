...
#endif
#endif
#if !defined(CONFIG_MACH_KLTE_CMCC) && !defined(CONFIG_MACH_KLTE_CTC)
...
static struct gpiomux_setting gpio_uart_sel = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_12MA,
	.pull = GPIOMUX_PULL_NONE,
};
...
#if defined(CONFIG_MACH_KLTE_VZW)
static struct msm_gpiomux_config ssp_config_vzw_rev05[] __initdata = {
	{
		.gpio = 49,
		.settings = {
			[GPIOMUX_ACTIVE] = &ssp_setting[3],
			[GPIOMUX_SUSPENDED] = &ssp_setting[3],
		},
	},
};
#endif
...
#if defined(CONFIG_MACH_K3GDUOS_CTC)
static struct gpiomux_setting mclk3_config_setting = {
	.func = GPIOMUX_FUNC_1,
	.drv = GPIOMUX_DRV_12MA,
	.pull = GPIOMUX_PULL_NONE,
};

static struct msm_gpiomux_config mclk3_config_ctc_rev02[] __initdata = {
	{
		.gpio = 18,
		.settings = {
			[GPIOMUX_ACTIVE] = &mclk3_config_setting,
			[GPIOMUX_SUSPENDED] = &mclk3_config_setting,
		},
	},
};
#endif
...
#if defined(CONFIG_MACH_KLTE_VZW)
	if (system_rev > 4)
		msm_gpiomux_install(ssp_config_vzw_rev05,
			ARRAY_SIZE(ssp_config_vzw_rev05));
#endif
#endif
...
#if defined(CONFIG_MACH_K3GDUOS_CTC)
	if (system_rev >= 6)	{
		msm_tlmm_misc_reg_write(TLMM_SPARE_REG, 0x8);
		msm_gpiomux_install(mclk3_config_ctc_rev02,
			ARRAY_SIZE(mclk3_config_ctc_rev02));
	}
#endif
